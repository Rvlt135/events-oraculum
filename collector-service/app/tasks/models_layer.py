    ...
    try:
        ...
        for slug_key, comp_config in api_football_policy.competitions.items():
            try:
                ...
                events, team_ids = await service_team_features.get_events_by_competition(competition_id,
                                                                                         seasons_current)
                if not events:
                    logger.info("no_events_found", slug_key=slug_key, competition_id=str(competition_id),
                                season=seasons_current)
                    continue
                ...
                features = await service_layer_models.extract_features_for_elo_build(events, team_ids, competition_id,
                                                                                     seasons_current)
                if not features:
                    logger.info("no_fixtures_rows", slug_key=slug_key, competition_id=str(competition_id),
                                season=seasons_current)
                    continue
