...
def _key_fixtures_teams(league_id: str, season: int) -> str:
    """Generate Redis key for competitions catalog teams by slug_key."""
    teams_key = "{league_id}:{season}:teams"
    return f"{KEY_PREFIX_FIXTURES}:{teams_key.format(league_id=league_id, season=season)}"
...
class StandingsFootballCache:
    ...
    async def save_fixtures_items(self, league_id: str, season: int, items: list[dict], ttl: int = CATALOG_TTL_SEC) -> None:
